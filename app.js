...
const TEXT = {
  lt: {
    appDescription:
      "LT: Interaktyvi navigacijos programa jūrai, GPS/GLONASS ir maršrutų planavimo funkcijomis. EN: Responsive sea navigation app with real-time location and route planning.",
    gpsTitle: "Realaus laiko GPS / Real-time GPS",
    gpsStatusWaiting: "Laukiama GPS duomenų / Waiting for GPS...",
    gpsStatusActive: (lat, lng) =>
      `GPS status: veikia / active — ${lat.toFixed(5)}, ${lng.toFixed(5)}`,
    gpsStatusError: (msg) => `GPS klaida / GPS error: ${msg}`,
    routeDistanceTitle: "Maršruto atstumas",
    routeEmpty: "Maršrutas tuščias / Route empty",
    routeDistance: (km) => `Maršrutas: ${km.toFixed(2)} km / Route distance`,
    waypointModeHint:
      "Spustelėkite žemėlapį, kad pridėtumėte tašką / Click map to add waypoint.",
    offlineTitle: "Offline funkcijos",
    offlineDescription:
      "LT: Išsaugokite pasirinktą sritį naudodami vietinę saugyklą. EN: Save selected area for offline use.",
    offlineWaiting: "Laukia / Waiting",
    offlineSaved: "Offline zona išsaugota / Offline area saved.",
    offlineLoaded: "Offline zona įkelta / Offline area loaded.",
    offlineNoData: "Nėra išsaugotų offline duomenų / No offline data found.",
    boatSettingsTitle: "Laivo nustatymai / Boat settings",
    chartsTitle: "Žemėlapiai / Charts",
    mapLayersTitle: "Žemėlapio sluoksniai / Map layers",
    footer:
      "LT: Palieskite žemėlapį, kad pridėtumėte maršruto tašką. EN: Tap the map to add route waypoints.",
  },
  en: {
    appDescription:
      "LT: Interaktyvi navigacijos programa jūrai, GPS/GLONASS ir maršrutų planavimo funkcijomis. EN: Responsive sea navigation app with real-time location and route planning.",
    gpsTitle: "Realaus laiko GPS / Real-time GPS",
    gpsStatusWaiting: "Waiting for GPS... / Laukiama GPS duomenų...",
    gpsStatusActive: (lat, lng) =>
      `GPS status: active — ${lat.toFixed(5)}, ${lng.toFixed(5)}`,
    gpsStatusError: (msg) => `GPS error: ${msg}`,
    routeDistanceTitle: "Maršruto atstumas",
    routeEmpty: "Route empty / Maršrutas tuščias",
...
function renderAllTexts() {
  ...
  const offlineDesc = document.getElementById("offline-description");
  if (offlineDesc) offlineDesc.textContent = t.offlineDescription;
  ...
  if (mapFooter) mapFooter.textContent = t.footer;
}
...
function createDepthMarkers() {
  ...
  points.forEach((point) => {
    L.circleMarker([point.lat, point.lng], {
      ...
      .bindPopup(`Gylis: ${point.depth} m<br>Depth: ${point.depth} m`)
      .addTo(depthLayer);
  });
}
...
function updatePositionMarker(lat, lng, accuracy) {
  if (!currentPosition.marker) {
    currentPosition.marker = L.circleMarker([lat, lng], {
      ...
      .addTo(map)
      .bindPopup("Jūsų vieta / Your location");
...
function geolocate() {
  if (!navigator.geolocation) {
    document.getElementById("gps-status").textContent =
      "GPS nepasiekiamas / GPS unavailable";
    return;
  }
  ...
  navigator.geolocation.watchPosition(
    ...
    (error) => {
      document.getElementById("gps-status").textContent =
        `GPS klaida / GPS error: ${error.message}`;
    },
    {
...
function refreshRoute() {
  ...
  if (positions.length >= 2) {
    ...
    document.getElementById("route-distance").textContent =
      `Maršrutas: ${total.toFixed(2)} km / Route distance`;
  }
}
...
function downloadOfflineArea() {
  ...
  document.getElementById("offline-status").textContent =
    "Offline zona išsaugota / Offline area saved.";
}
...
function loadOfflineArea() {
  ...
  if (!offlineData) {
    document.getElementById("offline-status").textContent =
      "Nėra išsaugotų offline duomenų / No offline data found.";
    return;
  }
  const data = JSON.parse(offlineData);
  map.setView([data.center.lat, data.center.lng], data.zoom);
  document.getElementById("offline-status").textContent =
    "Offline zona įkelta / Offline area loaded.";
}
...
function init() {
  ...
  geolocate();
  document.getElementById("gps-status").textContent =
    "Laukiama GPS duomenų / Waiting for GPS...";
}
